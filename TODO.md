...
### 問題
`Forest(dom; range = :integer, ev = :evtimes)` を呼ぶと
...
**A. C++ レベルでの専用 ifthenelse 操作を実装する**
`meddly_edge_ifthenelse(cond_bool, then_evp, else_evp, result_evp)` を
...
ただし「欠落ミンタームを 0 と区別したい」用途では意味が変わる。
...
この非対称性はユーザーが混乱しやすい。ドキュメント整備が望ましい。
...
| ファイル | 追加が必要な内容 |
|---|---|
| `c/meddly_c.h` | `meddly_edge_ifthenelse` の宣言 |
| `c/meddly_c.cpp` | `meddly_edge_ifthenelse` の実装（DD 直接走査） |
| `src/lowlevel.jl` | `_ll_edge_ifthenelse` ccall ラッパー |
